...
public class RemindersTest
{
	...
	{
		InventorySnapshot inv = new InventorySnapshot(0, 0, 0, false, 28, true, true, true, false, true, false, ItemID.ABYSSAL_LANTERN_MAGIC);
	...
	{
		return new InventorySnapshot(0, 0, 0, false, 28, chisel, pickaxe, false, false, lanternEquipped, lanternInBag, lanternId);
	}

	private static InventorySnapshot withActiveEssence(InventorySnapshot inv)
	{
		return new InventorySnapshot(inv.getDenseBlocks(), inv.getDarkBlocks(), inv.getFragments(), inv.isFragmentsConfirmed(), inv.getEmptySlots(),
